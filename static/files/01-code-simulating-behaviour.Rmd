...
download.file(paste0(url_data,"data.zip"), "./data/data.zip")
...
# Read in shapefiles defining OAs in Leeds 
# From CDRC https://data.cdrc.ac.uk/dataset/cdrc-2011-oac-geodata-pack-leeds-e08000035)
oa_boundaries <- st_read("./data/leeds_OAs/leeds_OAs.shp")
# Set CRS to OSGB.
oa_boundaries <- st_transform(oa_boundaries, crs=27700)
oa_boundaries  <- rmapshaper::ms_simplify(oa_boundaries, keep=0.1)
# For context, read in ward boundaries.
# From ONS open geography http://geoportal.statistics.gov.uk/datasets/07194e4507ae491488471c84b23a90f2_3
ward_boundaries <- st_read("https://opendata.arcgis.com/datasets/07194e4507ae491488471c84b23a90f2_3.geojson")
# Set CRS to OSGB.
ward_boundaries <- st_transform(ward_boundaries, crs=27700)
# Filter Leeds wards.
ward_boundaries <- ward_boundaries %>% filter(lengths(st_contains(., oa_boundaries))>0)
```
...
simulated_oac_age_sex <- rakeR::rk_rake(cons=temp_cons, inds=individuals %>% select(person_id, oac_grp, age_sex), 
...
# Generate choropleths. Ignore warnings.
plot <- oa_level_summary %>%
  group_by(statistic_type) %>%
  mutate(
    # Rescale summary stats between 0 and 1 for local scale on facet.
